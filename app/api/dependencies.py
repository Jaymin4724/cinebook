...
def get_admin_service(db: DBDep, redis: RedisDep, user_repo: UserRepoDep) -> AdminService:
    return AdminService(db=db, redis=redis, user_repo=user_repo)
...
def permission_required(permission: str):
    ...
    async def permission_dependency(
        ...
        async with db.begin():
            ...
            is_allowed = permission_repo.permission_check_repo(
                user_id=user_id,
...
def get_user_id(authorization: str) -> str:
    if authorization.startswith("bearer "):
        ...
        payload = decode_token(
            token=token, secret=settings.JWT_SECRET_REFRESH_KEY)
