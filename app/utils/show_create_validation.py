...
async def validate_start_time_of_show(
...
async def validate_category_price(
    ...
    screen_layout = await screen_repo.get_layout_by_screen_repo(
        screen_id=screen_id
    )
...
async def validate_movie_and_return_time(
    movie_id: str,
    movie_repo: MovieRepository
):
    
    movie_found = await movie_repo.get_movie_by_id(
        movie_id=movie_id
    )
...
async def validate_show_overlap(
    ...
    if previous_show:
        previos_show_end_time = previous_show.start_time.replace(tzinfo=timezone.utc) + timedelta(seconds=previous_show.movie.duration.total_seconds())
