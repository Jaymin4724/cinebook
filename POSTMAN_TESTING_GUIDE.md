...
# PHASE 9 — Cross-Cutting Security & Robustness

Run these spot checks on any protected endpoint (example: `GET /api/v1/users/me`):

| #  | Test                                                                                             | Expected                                                                             |
| -- | ------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------ |
| 1  | No`Authorization` header                                                                       | 401`{"detail": "Not authenticated"}`                                               |
| 2  | `Authorization: Bearer garbage`                                                                | 401`{"detail": "Invalid or expired token"}`                                        |
| 3  | Expired access token (wait out`JWT_ACCESS_TOKEN_EXPIRE_MINUTES`)                               | 401`{"detail": "Invalid or expired token"}`                                        |
| 4  | **Refresh** token used as Bearer access token                                              | 401`{"detail": "Invalid or expired token"}` (type check)                           |
| 5  | Revoked (logged-out) access token                                                                | 401`{"detail": "Token has been revoked"}`                                          |
| 6  | user token on`/admin/*` (all routes, including `GET /admin/movies`)                          | 403`{"detail": "Permission denied"}`                                               |
| 7  | user token on`/theatre-admin/*`                                                                | 403`{"detail": "Permission denied"}`                                               |
| 8  | theatre_admin token on`/admin/*`                                                               | 403`{"detail": "Permission denied"}`                                               |
| 9  | theatre_admin A operating on theatre_admin B's theatre/layout/screen/show                        | 404 (ownership scoping, not 403)                                                     |
| 10 | **Rate limit** (only if `ENV` ≠ `TESTING`): fire 15 rapid requests to any endpoint    | first ~10 succeed, then 429`{"detail": "Too many requests"}`; recovers ~1 req/10 s |
| 11 | Unknown route`GET /api/v1/nope`                                                                | 404`{"detail": "Not Found"}`                                                       |
| 12 | Wrong method (e.g.`GET /api/v1/auth/signin`)                                                   | 405`{"detail": "Method Not Allowed"}`                                              |
| 13 | Swagger`/docs` and OpenAPI `/openapi.json` load and list exactly the endpoints in this guide | 200                                                                                  |
...
- [ ] `GET /health` returns all three checks `ok`
- [ ] Migrations applied, DB seeded (`python -m app.scripts.seed_db`)
- [ ] Swagger UI matches implementation (no ghost/missing endpoints)

**Authentication**

- [ ] Send OTP works; OTP email delivered (10 min TTL, 3 tries, single-use)
- [ ] Signin auto-creates new users with role `user`
- [ ] Signin for existing users works
- [ ] Wrong OTP decrements tries; exhaustion invalidates OTP
- [ ] JWT access token authorizes protected endpoints
- [ ] Refresh rotates tokens; old refresh token rejected (reuse detection)
- [ ] Logout revokes access (and optional refresh) token
- [ ] Revoked/expired/invalid tokens rejected with correct 401s
- [ ] Refresh token cannot be used as an access token
- [ ] Google OAuth login round-trip works; state is single-use
- [ ] Soft-deleted users cannot log in (OTP or Google)

**RBAC**

- [ ] user / theatre_admin / admin permission matrix enforced (Phase 9 table)
- [ ] Cross-operator resource access returns 404 (no data leak)
- [ ] PATCH endpoints + verify-ticket work for admins/operators

**Admin**

- [ ] Create user with role (OTP-gated) works
- [ ] Create movie via OMDB by imdb_id AND by title; duplicates rejected
- [ ] Create theatre with operator linkage works; bad operator → 404
- [ ] Update theatre/movie (partial) works; immutable fields ignored
- [ ] List users/theatres/movies with pagination bounds (1 ≤ size ≤ 100)
- [ ] Soft delete movie/theatre works and hides them everywhere

**Theatre Admin**

- [ ] Create layout: polishing, seat numbering, categories, seat_mapping correct
- [ ] Create screen bound to owned theatre + its layout
- [ ] Create show: 2 h/3 day window, overlap guard, full category pricing enforced
- [ ] Update layout/screen names and show pricing
- [ ] my-theatres / my-screens scoped to the operator
- [ ] Delete screen/show works and re-delete → 404

**User Journey**

- [ ] Public browse: theatre→movies, movie→theatres, shows list (both path orders)
- [ ] Seat map shows Available/Locked/Booked with correct prices
- [ ] Seat lock: atomic all-or-nothing, 10-min expiry, blocks other users
- [ ] Booking requires own lock; bill computed correctly; 409 on double-book
- [ ] QR ticket email delivered; hash decodes and verifies
- [ ] Ticket verify: success once, rejected on reuse/cancel/expiry/wrong theatre
- [ ] Booking history + detail (ownership enforced)
- [ ] Cancel: ≥ 6 h rule, seats released in Redis and rebookable
- [ ] Profile get/update; account soft delete blocks re-login

**Integrations & Middleware**

- [ ] OMDB external API reachable with configured key
- [ ] Elasticsearch search returns synced movies + theatres; 503 when ES down
- [ ] Background tasks executed: OTP email, QR email, ES sync on create/update/delete
- [ ] Redis caches behave: OTP keys, layout cache (100 min), seat locks (10 min), token blacklist
- [ ] Rate limiter returns 429 under burst (when `ENV` ≠ `TESTING`)
- [ ] Global 500 handler masks internals (no stack traces in responses)

---

*Generated from code inspection of `app/api`, `app/services`, `app/repositories`, `app/schemas`, `app/middlewares`, `app/utils`, and seed scripts — every endpoint in `app/api/v1/*` plus `/health` is covered. No endpoints were invented.*
